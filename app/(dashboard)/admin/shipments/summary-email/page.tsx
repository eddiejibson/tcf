...
export default function ShipmentSummaryEmailPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<{ shipments: SummaryShipment[]; count: number; recipientCount: number } | null>(null);
  ...
  const fetchData = useCallback(async () => {
    const res = await fetch(`/api/admin/shipments/summary-email`);
  ...
  const hasShipments = (data?.count ?? 0) > 0;
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`/api/admin/shipments/summary-email`, {
          ...
          body: JSON.stringify({ subject: subject || undefined, intro, preview: true, imageUrls }),
        });
        if (res.ok) {
  ...
  const handleSend = async () => {
    ...
    const res = await fetch(`/api/admin/shipments/summary-email`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ subject: subject || undefined, intro, testEmails: emails, imageUrls }),
    });
  ...
  if (!data) return <div className="p-8 text-white/40">Failed to load shipments</div>;

  const dayLabel = (d: number) => (d <= 0 ? "Last day" : d === 1 ? "1 day left" : `${d} days left`);
  const dayColor = (d: number) => (d <= 1 ? "text-red-400" : d <= 3 ? "text-amber-300" : "text-green-400");
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white">Shipments Summary</h1>
        <p className="text-white/50 text-sm mt-1">{data.count} active shipment{data.count === 1 ? "" : "s"} · {data.recipientCount} recipients</p>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {/* Left: Compose */}
        <div className="space-y-5">
          {/* Active shipments in this summary */}
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[20px] shadow-2xl shadow-black/40 p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-white/40 text-[10px] uppercase tracking-wider font-medium">In This Summary ({data.count})</p>
              <button onClick={() => router.push(`/admin/shipments`)} className="text-[#0984E3] text-xs font-medium hover:text-[#0984E3]/80 transition-colors">Manage</button>
            </div>
            {data.count === 0 ? (
              <p className="text-white/30 text-sm">No active shipments. Set a shipment to ACTIVE with a future deadline for it to appear here.</p>
            ) : (
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[20px] shadow-2xl shadow-black/40 p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder={`${data.count} Open Shipment${data.count === 1 ? "" : "s"}`} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm placeholder-white/20 focus:outline-none focus:border-[#0984E3]/50" />
              </div>
              <div>
                <label className="text-white/40 text-[10px] uppercase tracking-wider font-medium block mb-1.5">Intro Message</label>
                <textarea value={intro} onChange={(e) => setIntro(e.target.value)} rows={4} className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white text-sm placeholder-white/20 focus:outline-none focus:border-[#0984E3]/50 resize-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[20px] shadow-2xl shadow-black/40 p-5">
            ...
            <button
              ...
              disabled={sending || !hasShipments || (testMode && !testEmails.trim())}
            ...
            >
              ...
              {sending ? "Sending..." : testMode ? "Send Test Email" : `Send to All (${data.recipientCount})`}
            </button>
            {!hasShipments && <p className="text-white/30 text-xs mt-2 text-center">No active shipments to send.</p>}
            {sendResult && (
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[20px] shadow-2xl shadow-black/40 p-5">
            ...
            <button
              onClick={() => {
                ...
                const link = `${baseUrl}/login?to=/shipments`;
              ...
              className={`w-full py-3 font-medium rounded-xl transition-all text-sm flex items-center justify-center gap-2 disabled:opacity-50 ${
                waCopied
      ...
      {confirmStep === 1 && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setConfirmStep(0)}>
          <div className="bg-[#1a1f26] border border-white/10 rounded-[20px] p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-white font-semibold text-lg text-center mb-2">Send summary to all customers?</h3>
            <p className="text-white/50 text-sm text-center mb-6">This will send a summary of <strong className="text-white">{data.count}</strong> active shipment{data.count === 1 ? "" : "s"} to <strong className="text-white">{data.recipientCount}</strong> trade customers. This action cannot be undone.</p>
      ...
      {confirmStep === 2 && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setConfirmStep(0)}>
          <div className="bg-[#1a1f26] border border-red-500/20 rounded-[20px] p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-white/50 text-sm text-center mb-4">Type <strong className="text-red-400">SEND</strong> below to confirm sending to {data.recipientCount} recipients.</p>
            <input
              value={confirmText}
