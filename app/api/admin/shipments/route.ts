...
export async function POST(request: NextRequest) {
  ...
  const { name, deadline, shipmentDate, freightCost, margin, status, products, sourceFilename, fractionalBagsEnabled, deliveryOptions, notes, currency, freightCurrency } = body;
  ...
  const shipment = repo.create({
    ...
    freightCurrency: freightCurrency?.trim() ? freightCurrency.trim() : null,
