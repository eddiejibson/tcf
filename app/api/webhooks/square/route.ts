...
async function sendPaidNotification(orderId: string) {
  try {
    ...
    if (fullOrder) {
      const totals = calculateOrderTotals(fullOrder.items, fullOrder.includeShipping, fullOrder.freightCharge, fullOrder.creditApplied, fullOrder.discountPercent, fullOrder.deliveryCharge);
