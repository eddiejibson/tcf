...
interface CatalogProductFormProps {
  onSuccess: (product: { id: string; name: string; price: number; type: string; categoryId: string; categoryName: string }) => void;
  onCancel?: () => void;
}

export default function CatalogProductForm({ onSuccess, onCancel }: CatalogProductFormProps) {
  ...
  // Find category name for onSuccess callback
  ...
  const handleSubmit = async () => {
    ...
    if (res.ok) {
      ...
      onSuccess({
        ...
        type,
        categoryId,
        categoryName: findCategoryName(categoryId),
      });
    }
    setSaving(false);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-5 overflow-visible relative z-10">
        <div>
